# job_seeker_ro_spider — STEFANINI Careers Romania Scraper
...
> **🌱 Acest repo este un scraper derivat.** Repo-ul original (template-ul) este [epam-systems-international-srl-nodejs-scraper](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper). Toate scraper-ele din ecosistemul peviitor.ro sunt derivate din acel pattern.
...
- Extrage job-uri din SmartSearchOnline (HTML/cheerio)
...
│   ├── validate-stefanini-jobs.js   # SOLR job URL validation script
│   ├── unit/
...
│   ├── e2e/
│   │   └── scraper.test.js     # Full pipeline tests with real STEFANINI API
│   └── consistency/
...
3. Scrapes current job listings from STEFANINI Careers
...
7. Generates [`docs/jobs.md`](https://sebiboga.github.io/stefanini-romania-srl-nodejs-scraper/jobs.md) with company info and all scraped jobs
8. Pushes test reports and `docs/jobs.md` to [`docs/`](https://sebiboga.github.io/stefanini-romania-srl-nodejs-scraper/)

### Test Automation

The `automation-testing.yml` workflow runs on every push and pull request. It:
1. Ensures STEFANINI exists in the company core
2. Runs unit, integration, e2e, and consistency tests
3. Validates data integrity in Solr
4. Pushes test reports to [`docs/test-results/`](https://sebiboga.github.io/stefanini-romania-srl-nodejs-scraper/test-results/)
...
Acest scraper respectă regulile din [robots.txt](https://jobs2.smartsearchonline.com/robots.txt) al SmartSearchOnline. Pentru analiza completă, vezi [ROBOTS.md](ROBOTS.md).

**Puncte cheie:**
- SmartSearchOnline are `Disallow: /` — întregul site e deschis doar pentru User-Agent-ul implicit
- În practică, serverul răspunde cu 200 OK cu User-Agent standard
- Scraperul face o singură cerere per scrape (o pagină cu toate job-urile) — comportament rezonabil

## Disclaimer

This scraper is designed for educational purposes and legitimate job data aggregation for the Romanian job market. Please respect STEFANINI's Terms of Service and robots.txt when using this scraper.
